# mem0plus Product Definition v1

Продуктовый документ, фиксирующий `v1`-рамку для `mem0plus` с приоритетом на реальную пользовательскую ценность, а не только на архитектурную полноту.
...
`mem0plus` — это `self-hosted`, часто `offline-first` memory layer для агентных платформ, который в первую очередь улучшает `персональную память ассистента`, а также поддерживает `scoped shared memory` на уровне проектов, разделяемую между несколькими агентами.
