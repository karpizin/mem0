...
|---|---|---|---|---|---|---|---|
| `BL-001` | Make OpenClaw plugin recall timeout configurable and instrumented | `p1` | `Adapters / OpenClaw Contract` | `F-2026-04-21-001` | `yes` | Implemented on 2026-04-21; rerun showed partial mitigation at 15000ms, and low-value skip policy is now in place. Compact legacy recall injection is now added too; a live sanity turn showed `injecting 2/4 memories ... 319 chars` and completed in ~13.9s. Next step is measuring whether plugin timeout frequency drops enough, or whether deeper recall-path trimming is still needed | `integration` |
...
- `which metrics / traces should be compared before vs after:` plugin timeout frequency, recall trace selection patterns, live scenario pass rate, user-facing injected memory quality, embedded agent timeout frequency, and prompt/system-payload size on heavy carryover or continuity turns
